...
#13/03/2017
#add 2x2, 3x3, 4x4 support
#add ability to run all permutations in one set with number a loops per permutation
...
class Application(tk.Frame):
	...
	def configSweep(self):
		...
		self.ser.write("smua.trigger.measure.i(defbuffer1)".encode())
		self.ser.write("smua.trigger.measure.action = smua.ENABLE".encode())
		...
		self.ser.write("smua.trigger.endsweep.action = smua.SOURCE_IDLE".encode())
		...
		self.ser.write(("bufferVar.fillcount = "+str(self.steps.get()+1)+"\n").encode())	#Set amount of points to store in memory (steps + 1)
		self.ser.write(("smua.measure.delay = "+str(self.delay.get())+"\n").encode())	#Set delay
		self.ser.write(("smua.trigger.arm.count = "+str(self.loops.get())+"\n").encode())	#Set delay
	...
	def runSweep(self):
		...
		self.ser.write(("printbuffer(1," + str(self.steps.get()) + ",defbuffer1)\n").encode())
