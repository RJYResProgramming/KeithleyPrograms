...
class Application(tk.Frame):
	...
	def configSweep(self):
		...
		self.ser.write("*RST".encode())	#Reset SMU
		self.ser.write(":SYST:BEEP:STAT 0".encode())	#Turn off beeper
		self.ser.write(":SENS:FUNC:CONC OFF".encode())
		self.ser.write(":SOUR:FUNC VOLT".encode())	#Source Voltage
		self.ser.write(":SENS:FUNC 'CURR:DC'".encode())	#Sense current (DC)
		self.ser.write((":SENS:CURR:PROT "+str(self.compliance.get())+"").encode())	#Set compliance (A)
		self.ser.write(":SENS:CURR:NPLC 0.05".encode())
		self.ser.write((":SOUR:VOLT:STAR "+str(self.start.get())+"").encode())	#Set sweep start point (V)
		self.ser.write((":SOUR:VOLT:STOP "+str(self.stop.get())+"").encode())	#Set sweep start point (V)
		self.ser.write((":SOUR:VOLT:STEP "+str(stepsize)+"").encode())	#Set sweep step size (V)
		self.ser.write(":SOUR:VOLT:MODE SWE".encode())	#Change to sweep mode
		self.ser.write(":SOUR:SWE:RANG AUTO".encode())	#Set source range AUTO/BEST/FIXED
		self.ser.write(":SOUR:SWE:SPAC LIN".encode())	#Set steps spacing LIN/LOG
		self.ser.write((":TRIG:COUN "+str(self.steps.get()+1)+"").encode())	#Set amount of points to store in memory (steps + 1)
		self.ser.write((":SOUR:DEL "+str(self.delay.get())+"").encode())	#Set delay
		self.ser.write(":SOUR:SWE:DIR UP".encode())	#Set sweep direction UP(start->stop)/DOWN(stop->start)
	...
	def configCustomSweep(self):
		...
		self.ser.write("*RST".encode())	#Reset SMU
		self.ser.write(":SYST:BEEP:STAT 0".encode())	#Turn off beeper
		self.ser.write(":SENS:FUNC:CONC OFF".encode()) #Turn off concurrent functions
		self.ser.write(":SOUR:FUNC VOLT".encode())	#Source Voltage
		self.ser.write(":SENS:FUNC 'CURR:DC'".encode())	#Sense current (DC)
		self.ser.write((":SENS:CURR:PROT "+str(self.compliance.get())+"").encode())	#Set compliance (A)
		self.ser.write(":SENS:CURR:NPLC 0.05".encode())
		self.ser.write(":SOUR:VOLT:MODE LIST".encode())	#Sweep mode: list
		self.ser.write((":SOUR:LIST:VOLT "+customSweepList+"").encode())	#List of values
		self.ser.write((":TRIG:COUN "+str(self.steps.get()+1)+"").encode())	#Set amount of points to store in memory (steps + 1)
		self.ser.write((":SOUR:DEL "+str(self.delay.get())+"").encode())	#Set delay
		self.ser.write(":SOUR:SWE:DIR UP".encode())	#Set sweep direction UP(start->stop)/DOWN(stop->start)		
...
plt.ion()
app = Application()
app.master.title('Keithley 2602B Sweeper')
app.mainloop()
