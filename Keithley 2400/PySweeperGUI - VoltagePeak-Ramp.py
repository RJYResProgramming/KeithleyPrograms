...
class Application(tk.Frame):
	def __init__(self, master=None):
		...
		self.NPLC.set(0.05)
		self.pulseV = tk.DoubleVar()
		self.pulseStart = tk.DoubleVar()
		self.pulseStart.set(0.0)
		self.pulseStop = tk.DoubleVar()
		self.pulseStop.set(0.0)
		self.pulseSteps = tk.IntVar()
		self.pulseSteps.set(100)
	...
	def createWidgets(self):
		...
		self.pulseStartLabel = tk.Label(text = "PULSE Start(V)")
		self.pulseStartLabel.grid(row = 4, column = 0, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.pulseStartEntry = tk.Entry(textvariable = self.pulseStart, width = 6, justify = tk.RIGHT)
		self.pulseStartEntry.grid(row = 5, column = 0, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.pulseStopLabel = tk.Label(text = "PULSE Stop(V)")
		self.pulseStopLabel.grid(row = 4, column = 1, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.pulseStopEntry = tk.Entry(textvariable = self.pulseStop, width = 6, justify = tk.RIGHT)
		self.pulseStopEntry.grid(row = 5, column = 1, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.loopsLabel = tk.Label(text = "LOOPS(#)")
		self.loopsLabel.grid(row = 4, column = 2, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.loopsEntry = tk.Entry(textvariable = self.loops, width = 6, justify = tk.RIGHT)
		self.loopsEntry.grid(row = 5, column = 2, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		
		self.delayVLabel = tk.Label(text = "PEAK DELAY (s)") # After peak delay before sweep
		self.delayVLabel.grid(row = 6, column = 0, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.delayVEntry = tk.Entry(textvariable = self.delayV, width = 6, justify = tk.RIGHT)
		self.delayVEntry.grid(row = 7, column = 0, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.delayLoopLabel = tk.Label(text = "SWEEP DELAY(s)") #After sweep Delay before nhext peak
		self.delayLoopLabel.grid(row = 6, column = 1, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
		self.delayLoopEntry = tk.Entry(textvariable = self.delayLoop, width = 6, justify = tk.RIGHT)
		self.delayLoopEntry.grid(row = 7, column = 1, sticky = tk.W+tk.E, padx = 5, pady = 5, in_ = self.settingsFrame)
	...
	def configSweep(self):
		...
		self.ser.write((":SENS:CURR:PROT "+str(self.compliance.get())+"\n").encode())	#Set compliance (A)
		self.ser.write(":SENS:CURR:NPLC 0.05\n".encode())
		self.ser.write((":SOUR:VOLT:STAR "+str(self.start.get())+"\n").encode())	#Set sweep start point (V)
		self.ser.write((":SOUR:VOLT:STOP "+str(self.stop.get())+"\n").encode())	#Set sweep start point (V)
	...
	def configCustomSweep(self):
		...
		self.ser.write(":SYST:BEEP:STAT 0\n".encode())	#Turn off beeper
		self.ser.write(":SOUR:FUNC VOLT\n".encode())	#Source Voltage
		self.ser.write(":SOUR:VOLT:MODE FIXED\n".encode())
		self.ser.write((":SOUR:VOLT:RANG 10\n").encode())
		self.ser.write((":SOUR:VOLT:LEV "+str(self.pulseV.get())+"\n").encode())	
		self.ser.write(":SENS:FUNC 'CURR'\n".encode())	#Sense current (DC)
		self.ser.write((":SENS:CURR:PROT "+str(self.compliance.get())+"\n").encode())	#Set compliance (A)
		self.ser.write(":SENS:CURR:NPLC 0.05\n".encode())		
	...
	def recoverData(self, data):
		...
		for ii in range(0, self.steps.get() + 1):
			...
			for jj in range(2,5):
				list.pop(0)	#...and discard the followinfg three
		return results
	...
	def loopSweeps(self):
		...
		for l in range(0,self.loops.get()):
			...
			self.pulseV.set(self.pulseStart.get()+ (l+1)*((self.pulseStop.get()-self.pulseStart.get())/self.loops.get()))
			self.configCustomSweep()
			#print(self.loopCount.get())
			#print(self.pulseV.get())
			time.sleep(self.delayLoop.get())
			self.ser.write(":OUTP ON\n".encode())
			self.ser.write(":INIT\n".encode())
			time.sleep(self.delayV.get())
